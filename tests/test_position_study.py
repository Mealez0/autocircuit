...
def test_generation_is_byte_stable_balanced_and_matched(tmp_path: Path) -> None:
    ...
    report = validate_matched_dataset(first)
...
def test_failed_records_prevent_end_to_end_eligibility(tmp_path: Path) -> None:
    class FailingAdapter(FakeAdapter):
        def score(self, examples: list[ExamplePair], batch_size: int):
            records = super().score(examples, batch_size)
            records[0] = make_failed_result(examples[0], "failure")
            return records
